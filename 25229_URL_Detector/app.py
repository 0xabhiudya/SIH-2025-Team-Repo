...
import ipaddress # ADDED: For robust IP/CIDR filtering
...
st.set_page_config(
    layout="wide",
    page_title="SIREN: URL Attack Detector",
    page_icon="🚨"
)
...
def run_analysis(uploaded_file):
    ...
    """
    with st.spinner(f"SIREN is analyzing packets from `{uploaded_file.name}`... This may take a moment."):
        ...
        try:
            ...
            # Note: The column name changed from 'Status' to 'AttackStatus' in the detector
            results_df, packet_count = detector.analyze_pcap(tmp_file_path)
        finally:
            os.unlink(tmp_file_path)

    # Ensure timestamp is in a readable format
...
# --- IP Range Filtering Function (NEW) ---
def filter_by_ip_range(df: pd.DataFrame, ip_filter: str) -> pd.DataFrame:
    ...
    try:
        # 1. Check for CIDR (IP Range) format
        network = ipaddress.ip_network(ip_filter, strict=False)
        
        # Create a boolean series: True if IP is in the network
        in_range = df['SourceIP'].apply(lambda ip: ipaddress.ip_address(ip) in network if pd.notna(ip) else False)
        return df[in_range]
        
    except ValueError:
        # 2. If it's not a valid CIDR, treat it as a partial match (e.g., '192.168.1')
...
def display_sidebar(df: pd.DataFrame) -> tuple:
    ...
    # Filter by attack status (Successful/Attempted/Brute Force Detected)
    ...
    # Filter by source IP (Updated hint for CIDR support)
    source_ip_filter = st.sidebar.text_input(
        "Filter by Source IP (CIDR or partial match):", # UPDATED HINT
        placeholder="e.g., 192.168.1.1 or 10.0.0.0/8" # UPDATED PLACEHOLDER
    )
...
def display_metrics(df: pd.DataFrame, packet_count: int):
    ...
    # Metrics updated to use the new 'AttackStatus' column and track Brute Force
...
def display_dashboard(filtered_df: pd.DataFrame, packet_count: int):
    ...
    with tab1:
        ...
        # Define color scale for the new status, highlighting Brute Force in red
        color_scale = alt.Scale(
            domain=['Successful', 'Attempted', 'Brute Force Detected'], 
            range=['#28a745', '#ffc107', '#dc3545'] # Green, Yellow, Red
        )

        # Create a stacked bar chart for Successful vs. Attempted attacks
    ...
    with tab2:
        ...
        st.write("Browse, sort, and filter the raw attack data detected in the file.")
        # Display DataFrame using the new 'AttackStatus' column
...
def display_welcome_page():
    """Displays the initial welcome page and file uploader."""
    st.title("🚨 SIREN: URL-Based Cyber Attack Detection System")
    st.write(
        """
        Welcome to **SIREN**! This tool analyzes network traffic files (`.pcap`, `.pcapng`)
        to detect a variety of URL-based cyber attacks by correlating requests and responses.
        """
    )
    
    with st.container(border=True):
        st.subheader("Get Started")
        st.write("Upload a `.pcap` or `.pcapng` file to begin your analysis.")
...
def main():
    ...
    if 'analysis_results' not in st.session_state:
        ...
        # 1. Apply Attack Type filter
        filtered_df = df_results[
            (df_results['AttackType'].isin(selected_attacks))
        ]
        ...
        # 3. Apply the new complex IP range filter (CIDR/partial match)
